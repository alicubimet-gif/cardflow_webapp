  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-4">
        ...
        {/* Search & Filter */}
        <div className="flex flex-col md:flex-row gap-2 md:gap-2.5 w-full">
          <input
            type="text"
            placeholder="Search employee by name…"
            value={recordSearch}
            onChange={e => setRecordSearch(e.target.value)}
            className="w-full md:flex-1 h-11 px-4 border border-[#DFE4EA] focus:border-[#2563EB] rounded-xl text-sm focus:outline-none"
          />
          <select
            value={recordFilterStatus}
            onChange={e => setRecordFilterStatus(e.target.value)}
            className="w-full md:w-[200px] shrink-0 h-11 px-4 border border-[#DFE4EA] focus:border-[#2563EB] bg-white rounded-xl text-sm text-slate-700 focus:outline-none cursor-pointer text-center"
