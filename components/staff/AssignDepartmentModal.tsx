...
interface AssignDepartmentModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAssign: (selectedDepartmentIds: string[]) => Promise<void>;
  departmentsList: any[];
...
export function AssignDepartmentModal({
  ...
  useEffect(() => {
    if (isOpen) {
      // Find departments that are currently assigned
      const departmentIds = currentAssignments
        .filter(a => a.assignment_level === 'department' && a.department)
        .map(a => String(a.department.id || a.department));
      setSelectedIds(departmentIds);
    }
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in duration-200">
        ...
        <div className="p-6 space-y-4 max-h-[50vh] overflow-y-auto">
          <p className="text-[11px] font-semibold text-slate-500">
            Assigning individual departments grants access only to the selected departments within branches.
          </p>
          {departmentsList.length === 0 ? (
            <p className="text-xs text-slate-400 italic text-center py-4">No departments available.</p>
          ) : (
            <div className="space-y-2">
              {departmentsList.map(d => {
                const branchId = d.branch || d.branchId || d.branch_id;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-xs">
                      <span className="text-slate-400 mr-1.5 font-bold">[{branchName}]</span>
                      <span>{d.name}</span>
                    </div>
                  </button>
                );
