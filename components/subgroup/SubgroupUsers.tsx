...
export function SubgroupUsers({
  userList,
  allAssignmentsList,
  subgroupId,
  onViewUser
}: SubgroupUserProps) {
  // Filter user assignments & user members assigned to this subgroup
  const divAssignments = allAssignmentsList.filter(
    a => a.assignment_level === 'subgroup' && String(a.subgroup || a.subgroup_id) === String(subgroupId)
  );
  ...
  return (
    <div className="space-y-[16px]">
      ...
      >
        Subgroup User ({divUser.length})
      </h3>

      {divUser.length === 0 ? (
        <div className="bg-white border border-[#DFE4EA] rounded-[16px] p-5 text-center text-xs text-slate-400 italic font-medium">
          No user members assigned directly to this subgroup.
        </div>
      ) : (
        <div className="space-y-[16px]">
          {divUser.map(st => {
            const name = st.name || st.full_name || 'User operator';
