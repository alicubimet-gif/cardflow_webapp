...
import { MM_TO_PX, getAutoFitFontSize } from '../../utils/unit-conversion';
...
export function formatDateOfBirth(value: unknown) {
...
export function getFieldValue(fieldId: string, record: any): any {
  if (!record) return undefined;
  const normalizedKey = normalizePlaceholderKey(fieldId);
  ...
  const customData = record.custom_data || record.data || record.custom_fields || {};
  if (customData[normalizedKey] !== undefined && customData[normalizedKey] !== null) {
    return customData[normalizedKey];
  }
  if (customData[fieldId] !== undefined && customData[fieldId] !== null) {
    return customData[fieldId];
  }
  return undefined;
}

export function resolveTemplateText(text: string, record: any): string {
  if (!text) return '';

  const normalized = normalizePlaceholderKey(text);
  const directValue = getFieldValue(normalized, record);
  if (directValue !== undefined && directValue !== null && directValue !== "") {
    return String(directValue);
  }

  const exactValue = getFieldValue(text, record);
  ...
  return text.replace(
    ...
    (_, label: string) => {
      ...
      const value = getFieldValue(key, record) ?? getFieldValue(label, record);
      ...
      {sortedElements.map((el) => {
        ...
        const renderContent = () => {
          ...
          if (typeUpper === 'TEXT' || typeUpper === 'FIELD') {
            ...
            // Priority 1: Explicit field key/id mapping
            if (activeKey && getFieldValue(activeKey, recordData) !== undefined && getFieldValue(activeKey, recordData) !== null && getFieldValue(activeKey, recordData) !== '') {
              const val = getFieldValue(activeKey, recordData);
              resolvedText = activeKey === 'date_of_birth' ? formatDateOfBirth(val) : String(val);
            } 
            // Priority 2: Placeholder parsing & template text resolution
            else {
              resolvedText = resolveTemplateText(rawText, recordData);
              if (!resolvedText && rawText.includes('{{')) {
                resolvedText = resolveTokens(rawText, recordData);
              }
            }

            // Hide the element entirely if it represents an empty/unresolved placeholder or variable
            const isPlaceholderOrVar = (rawText.startsWith('[') && rawText.endsWith(']')) || rawText.includes('[') || rawText.includes('{{');
            if (isPlaceholderOrVar && (!resolvedText || !resolvedText.trim())) {
              return null;
            }
