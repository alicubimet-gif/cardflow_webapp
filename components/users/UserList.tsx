...
import { Search, PlusCircle, KeyRound, Edit, UserMinus, UserCheck, Eye, Users, Mail } from 'lucide-react';
...
export const UserList = React.memo(function UserList({
  ...
  const getAssignmentsCount = (userId: string) => {
    return allAssignmentsList.filter(
      a => String(a.user?.id || a.user || a.user_id) === String(userId)
    ).length;
  };

  // Filter user list based on search query and status filter
  const filteredUser = userList.filter(st => {
    ...
    <div className="space-y-4">
      ...
      {filteredUser.length === 0 ? (
        <EmptyState
          ...
          description="Create staff accounts for your organization."
        />
      ) : (
        <div>
          {/* Mobile grid view */}
                    ...
                    <tr key={st.id} className="hover:bg-slate-50/40 transition-colors">
                      <td className="py-4 px-6 font-bold text-slate-900">
                        ...
                        >
                          ...
                          <span className="truncate">{st.name || st.full_name || '-'}</span>
                        </button>
                      </td>
                      <td className="py-4 px-6 text-slate-500">{st.email || st.email_address || '-'}</td>
                      ...
                      <td className="py-4 px-6 text-right space-x-1 whitespace-nowrap">
                        <button
                          onClick={() => onView(st)}
                          className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all cursor-pointer inline-flex items-center justify-center"
                        ...
                        <button
                          onClick={() => onEdit(st)}
                          className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all cursor-pointer inline-flex items-center justify-center"
                          title="Edit Details"
                        >
                          <Edit size={15} />
                        </button>
                        {onResendInvite && (
                          <button
                            onClick={() => onResendInvite(st)}
                            className="p-1.5 text-slate-400 hover:text-violet-600 hover:bg-violet-50 rounded-lg transition-all cursor-pointer inline-flex items-center justify-center"
