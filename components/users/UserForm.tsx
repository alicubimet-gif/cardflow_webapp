...
export function UserForm({
  ...
  useEffect(() => {
    if (isOpen) {
      if (editingUser) {
        setName(editingUser.name || editingUser.full_name || '');
        setEmail(editingUser.email || editingUser.email_address || '');
        setPhone(editingUser.phone || '');
        setStatus(editingUser.status === 'active' || editingUser.is_active ? 'active' : 'inactive');
  ...
  }, [isOpen, editingUser, initialAssignments]);
  ...
  const getBranchLabel = (dept: any) => {
    const branchId = dept.group || dept.branchId || dept.branch_id;
    return getGroupName(branchId);
  };

  const getClassLabel = (div: any) => {
    const classId = div.group || div.classId || div.class_id;
    return getGroupName(classId);
  };
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in duration-200">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <h3 className="font-extrabold text-sm text-slate-900">
            {editingUser ? 'Update User Profile' : 'Create User Member'}
          </h3>
          <button 
            type="button"
            onClick={onClose}
            ...
            {!isOrganization ? (
              ...
              <div>
                ...
                <SearchableMultiSelect
                  options={subgroupsList.map(dept => ({
