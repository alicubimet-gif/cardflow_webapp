...
export function UserResetPasswordModal({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit(user.id, password.trim()).then(() => {
      setPassword('');
    });
  };
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-in fade-in duration-200">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4" noValidate>
          <div className="p-3 bg-blue-50 border border-blue-100 rounded-xl text-blue-800 text-[10px] font-bold">
            Resetting password for: <span className="font-extrabold text-slate-900">{user.name || user.full_name}</span>
          </div>
...
export type UserResetPasswordModalType = typeof UserResetPasswordModal;
