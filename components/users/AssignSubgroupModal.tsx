...
interface AssignSubgroupModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAssign: (selectedSubgroupIds: string[]) => Promise<void>;
  subgroupsList: any[];
...
export function AssignSubgroupModal({
  isOpen,
  onClose,
  onAssign,
  subgroupsList,
  ...
  getGroupName,
  isSubmitting = false
}: AssignSubgroupModalProps) {
  const [selectedIds, setSelectedIds] = useState<string[]>([]);

  useEffect(() => {
    if (isOpen) {
      // Find subgroups that are currently assigned
      const subgroupIds = currentAssignments
        .filter(a => a.assignment_level === 'subgroup' && a.subgroup)
        .map(a => String(a.subgroup.id || a.subgroup));
      setSelectedIds(subgroupIds);
    }
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in duration-200">
        ...
        <div className="p-6 space-y-4 max-h-[50vh] overflow-y-auto">
          <p className="text-[11px] font-semibold text-slate-500">
            Assigning individual subgroups grants access only to the selected subgroups within classes.
          </p>
          {subgroupsList.length === 0 ? (
            <p className="text-xs text-slate-400 italic text-center py-4">No subgroups available.</p>
          ) : (
            <div className="space-y-2">
              {subgroupsList.map((d: any) => {
                const groupId = d.group || d.groupId || d.group_id;
                const className = getGroupName(groupId);
