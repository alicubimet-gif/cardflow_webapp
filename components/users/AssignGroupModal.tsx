...
interface AssignGroupModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAssign: (selectedGroupIds: string[]) => Promise<void>;
  groupsList: any[];
...
export function AssignGroupModal({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const groupIds = currentAssignments
        .filter(a => a.assignment_level === 'group' && a.group)
        .map(a => String(a.group.id || a.group));
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in duration-200">
        ...
        <div className="p-6 space-y-4 max-h-[50vh] overflow-y-auto">
          <p className="text-[11px] font-semibold text-slate-500">
            Assigning a class grants access to all subgroups and students within that class.
          </p>
          {groupsList.length === 0 ? (
