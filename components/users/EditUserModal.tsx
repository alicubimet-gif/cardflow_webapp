...
export function EditUserModal({
  ...
  const orgType = (authUser as any)?.organization?.organization_type || authUser?.organization_type;
  ...
  useEffect(() => {
    if (isOpen && user) {
      ...
      setStatus(user.status === 'active' || user.is_active ? 'active' : 'inactive');
      setRole(user.role || 'ORGANIZATION_STAFF');
    ...
    try {
      ...
      if (err?.response?.data) {
        ...
        if (Object.keys(backendErrors).length > 0) {
          ...
          setError(errData.detail || errData.message || 'Failed to update user profile.');
        }
      } else {
