...
interface AssignUserModalProps {
  ...
  targetType: 'class' | 'division' | 'branch' | 'department' | 'group' | 'subgroup' | string;
...
export function AssignUserModal({
  ...
  useEffect(() => {
    if (isOpen && targetId) {
      const activeUserIds = currentAssignments
        .filter(a => {
          if (['class', 'group', 'branch'].includes(targetType)) {
            return ['class', 'group', 'branch'].includes(a.assignment_level) && String(a.group || a.school_class || a.class_id || a.branch || a.branch_id) === String(targetId);
          }
          if (['division', 'subgroup', 'department'].includes(targetType)) {
            return ['division', 'subgroup', 'department'].includes(a.assignment_level) && String(a.subgroup || a.division || a.division_id || a.department || a.department_id) === String(targetId);
          }
          return false;
        })
        .map(a => String(a.user || a.user_id || a.staff || a.staff_id));
      setSelectedIds(activeUserIds);
    }
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
          {userList.length === 0 ? (
            <p className="text-sm text-slate-500 text-center">No user members found.</p>
          ) : (
                ...
                return (
                  <label key={st.id} className="flex items-center gap-3 p-3 border border-slate-100 hover:bg-slate-50 rounded-xl cursor-pointer transition-colors">
                    <input
                      type="checkbox"
                      checked={isChecked}
                      onChange={() => toggleUser(String(st.id))}
