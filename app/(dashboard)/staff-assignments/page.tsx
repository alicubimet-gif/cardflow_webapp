...
import { Search, Plus, Edit, Trash2, Eye, Calendar, User, XCircle } from 'lucide-react';
import { useAuth } from '@/context/auth-context';
import { useOrgLabels } from '@/hooks/useOrgLabels';
import { useGroups } from '@/hooks/queries/useGroups';
import { useAllSubgroups } from '@/hooks/queries/useSubgroups';
import {
  useStaffAssignmentsList,
  useDeleteStaffAssignment,
} from '@/hooks/queries/useStaffAssignments';
import { StaffAssignmentFormDialog } from '@/components/staff-assignments/StaffAssignmentFormDialog';
...
export default function StaffAssignmentsPage() {
  ...
  // Queries
  const { data: assignments = [], isLoading, error } = useStaffAssignmentsList({
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap bg-white border border-slate-200/60 p-5 rounded-2xl shadow-xs">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">Manage staff assignments to organizational levels.</p>
        </div>
        <button
          onClick={handleAdd}
      ...
      {isLoading ? (
        ...
        <div className="bg-red-50 text-red-800 p-6 rounded-2xl border border-red-200">
          <p className="font-semibold text-sm">Failed to load staff assignments. Please try again.</p>
        </div>
      ) : assignments.length === 0 ? (
        <div className="bg-white border border-slate-200/60 rounded-2xl p-12 text-center shadow-xs">
          <p className="text-slate-500 text-sm font-medium">No staff assignments found.</p>
          <button
            onClick={handleAdd}
