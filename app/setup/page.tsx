...
function SetupPasswordInner() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!token || !uidb64) {
      setError('Setup token or link is missing. Please check your invitation email.');
      return;
    }
    ...
    try {
      await AuthApi.completePasswordSetup({
        uidb64,
        token,
        password,
      ...
      setSuccess(true);
      await checkSession(); // Load user profile into context
      setTimeout(() => {
        router.push('/');
      }, 1500);
    } catch (err: any) {
      setError(err?.response?.data?.message || err?.response?.data?.detail || err?.message || 'Failed to complete setup.');
    } finally {
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        <div className="flex justify-center mb-6">
          <Image
            ...
            height={70}
            priority
            className="w-auto h-auto object-contain select-none pointer-events-none max-w-[150px] sm:max-w-[180px] md:max-w-[220px]"
        ...
        <div className="bg-white py-8 px-4 border border-slate-200 sm:rounded-2xl sm:px-10 shadow-sm">
          ...
          {success ? (
            <div className="text-center py-4">
              ...
              <h3 className="text-sm font-bold text-slate-900 mb-2">Account Setup Complete!</h3>
              <p className="text-xs text-slate-600 leading-relaxed mb-6">
                Your password has been securely set. Redirecting to your dashboard...
              </p>
              <button 
                onClick={() => router.push('/')}
